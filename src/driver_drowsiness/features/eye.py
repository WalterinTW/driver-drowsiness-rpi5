...
def calculate_ear(points: Iterable[Point]) -> float:
    """Calculate eye aspect ratio from six ordered eye landmark points."""
    landmarks = _validate_points(points, expected_count=6)
    horizontal = _distance(landmarks[0], landmarks[3])
    if horizontal == 0.0:
        raise ValueError("Eye horizontal distance must be greater than zero.")

    vertical_1 = _distance(landmarks[1], landmarks[5])
    vertical_2 = _distance(landmarks[2], landmarks[4])
...
def _validate_points(points: Iterable[Point], expected_count: int) -> list[tuple[float, ...]]:
    ...
    if len(landmarks) != expected_count:
        raise ValueError(f"EAR calculation requires exactly {expected_count} points.")
